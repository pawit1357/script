SELECT * FROM (SELECT
                        ROW_NUMBER() OVER(ORDER BY LG.TxnID ASC) AS SEQ,
                        LG.ReqID,
                        LG.TxnID,
                        O.ObjectiveName,
                        LG.TxnDate,
                        ...
                        tStatus.GUARANTEE,
                        tStatus.CANCEL,
						ISNULL(tUsrOwner.USER_FU, '') as USER_OWNER_FU,
                        ISNULL(tUsrOwner.USER_FORM, '') as USER_OWNER_FORM,----- XXXXXXXX
                        ISNULL(tUsrOwner.USER_SETUP, '') as USER_OWNER_SETUP,
                        ISNULL(tUsrOwner.USER_ISSUE, '') as USER_OWNER_ISSUE,
                        ISNULL(tUsrOwner.USER_GUARANTEE, '') as USER_OWNER_GUARANTEE,
                        ISNULL(tUsrOwner.USER_CANCEL, '') as USER_OWNER_CANCEL,
                        ISNULL(tUsr.USER_FU, '') as USER_FU,
                        ISNULL(tUsr.USER_FORM, '') as USER_FORM,----- XXXXXXXX
             ...
             FROM TxnLG LG
                         ...
                         LEFT JOIN Ref_OCCode OC ON OC.OCCode = LG.DestOCCode
             ...
             LEFT JOIN
             (
             SELECT TxnID, [LG Follow Up] as USER_FU, [LG Form] as USER_FORM,[LG Setup] AS USER_SETUP,[LG Issue] AS USER_ISSUE,[LG Confirm] AS USER_GUARANTEE,[LG Cancel] AS USER_CANCEL FROM(SELECT TxnID, LGTeam, UserCode FROM LGWorkFlow ) ps PIVOT(MAX (LGTeam) FOR LGTeam IN ( [LG Follow Up], [LG Form],[LG Setup],[LG Issue],[LG Confirm],[LG Cancel])) AS pvt2
             ) AS tUsrOwner on tUsrOwner.TxnID = LG.TxnID
             LEFT JOIN
             (
...
--select * from [dbo].[LGWorkFlow] where TxnID='CPSLG190800018'