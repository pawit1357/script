SELECT * FROM (
SELECT ROW_NUMBER() OVER(ORDER BY LG.TxnID ASC) AS SEQ,
LG.ReqID,
LG.TxnID,
O.ObjectiveName,
LG.TxnDate,
LG.TxnStatus,
S.StatusDescription,ISNULL(LG.UserECU,'') UserECU,
LG.PartyName,LG.ObjectiveID,
...
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Follow Up' AND TxnID = LG.TxnID) AS USER_FU_CHECKER,
(SELECT ISNULL(LGWorkFlow.UserCode,'') FROM LGWorkFlow WHERE LGTeam = 'LG Form' AND TxnID = LG.TxnID) AS USER_FORM,
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Form' AND TxnID = LG.TxnID) AS USER_FORM_CHECKER,
(SELECT ISNULL(LGWorkFlow.UserCode,'') FROM LGWorkFlow WHERE LGTeam = 'LG Setup' AND TxnID = LG.TxnID) AS USER_SETUP,
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Setup' AND TxnID = LG.TxnID) AS USER_SETUP_CHECKER,
(SELECT ISNULL(LGWorkFlow.UserCode,'') FROM LGWorkFlow WHERE LGTeam = 'LG Issue' AND TxnID = LG.TxnID) AS USER_ISSUE,
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Issue' AND TxnID = LG.TxnID) AS USER_ISSUE_CHECKER,
(SELECT ISNULL(LGWorkFlow.UserCode,'') FROM LGWorkFlow WHERE LGTeam = 'LG Confirm' AND TxnID = LG.TxnID) AS USER_GUARANTEE,
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Confirm' AND TxnID = LG.TxnID) AS USER_GUARANTEE_CHECKER,
(SELECT ISNULL(LGWorkFlow.UserCode,'') FROM LGWorkFlow WHERE LGTeam = 'LG Cancel' AND TxnID = LG.TxnID) AS USER_CANCEL,
(SELECT ISNULL(LGWorkFlow.UserChecker,'') FROM LGWorkFlow WHERE LGTeam = 'LG Cancel' AND TxnID = LG.TxnID) AS USER_CANCEL_CHECKER 
FROM TxnLG LG 
...
LEFT JOIN Ref_LGStatus S ON S.StatusNo = LG.TxnStatus) TXN 
WHERE  TXN.TxnID='CPSLG190300088';
